...
This challenge is overcome by applying a *height* value based on the `clientHeight` of the content
and then set it to `auto` as soon as the transition is complete so that further changes
...
The transition *duration* default value is `300ms` and is controlled by the
[CSS custom property](https://developer.mozilla.org/en-US/docs/Web/CSS/--*) `--duration` for the `.collapsible` selector.
This allows setting the *duration* value globally for all *Collapsible* components or for a specific one,
if it has its own `className`, or by inline `style`:

```jsx
<Collapsible style={{'--duration':'1s'}}>
```
...
`onTransitionEnd`|func|`() => null`|Specifies a callback function after the `transform` property transition is finished *expanding* or *collapsing*. Receives the event object as an argument.
