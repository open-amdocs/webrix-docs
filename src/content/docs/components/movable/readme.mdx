...
The `<Movable/>` component can be used to create movable elements. As opposed to a draggable element,
the `<Movable/>` component does not require a source/target configuration. Instead, it supports 3 custom
events (`onBeginMove`, `onMove`, `onEndMove`) through which you can control the movement.
...
In this example above we're using the `Movable.Constraints.snap(...)` constraint to make the movable
element snap to a 20px by 20px grid.
...
In this example we're using the `Movable.Constraints.contain(...)` constraint, to limit the movement
